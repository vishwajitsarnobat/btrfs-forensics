...
- **Schema version: 7.** `PRAGMA user_version` and `scan_runs.schema_version` hold it. A change to
...
| `chunks.problems` | JSON list; non-empty exactly when `accepted` is 0. In the `dev_extents` map: device extents of different lengths or overlapping, a striped profile (a DEV_EXTENT does not record the stripe order, and none is guessed), more device extents than the profile has copies (the address was reused), two or more device extents that no single dev-tree leaf holds together (chunks that had the address at different times, not copies of one), a length the block group contradicts, or no block-group item on a filesystem with several devices |
...
| `summary` | JSON: `artifacts` (count per status), `by_source` (count per source kind and status), `orphan_leaves` (leaves no root tree leads to), `fragments` and `fragments_found` (fragments read with `--graph`, and how many there are; at most 4096 are read, newest first), `log_trees` (log trees read with `--logs`), `bytes_written`, `gaps` (per root and tree, the blocks the tree walk could not follow) |
...
| `artifact_id` | row id |
| `recovery_id` | the run |
| `source_kind` | how the inode was reached. `anchored_root`: a walk down from a cataloged root tree. `orphan_node`: a leaf read on its own (`recover --orphans`), either a file-tree leaf that no ROOT_ITEM in any scanned root-tree leaf leads to, or a leaf of a dropped log tree (`tree_id` -6). `orphan_item`: found by an anchored walk, in a tree that lists the inode under the kernel's ORPHAN_ITEM (unlinked while open). `log_tree` (`recover --logs`): an inode of a subvolume's log tree, live or dropped, reached through the log root tree whose ROOT_ITEM names it (key offset = the subvolume), and replayed read-only over the subvolume's tree in the newest cataloged state older than the log; `tree_id` is then the subvolume, `root_bytenr` the log tree's root. `orphan_graph` (`recover --graph`): assembled from blocks no root tree leads to, on the evidence `joined` records: every inode of a fragment, and an inode of a lone leaf that a join contributed to (a continuation in another leaf, or the name of a parent directory) |
| `source` | the root as the user names it: `current`, `backup:GEN` or `state:ID`; `orphan_node:BYTENR` for a lone leaf; `log:BYTENR@GEN` for a log tree; `fragment:BYTENR@GEN` for a fragment, named after its top block: a file-tree internal node that no scanned key pointer, no ROOT_ITEM and no superblock slot names. A fragment is a tree version that was written within a transaction and replaced before the commit, or one whose every root tree is lost (`m5_delsubvol_lost_items`). Its blocks may have been written at different moments of one transaction: it cannot be taken for a committed state |
...
| `joined` | JSON list of the joins the artifact rests on (`recover/graph.py`); empty unless `source_kind` is `orphan_graph`. Each has `kind` and `evidence` (the grounds, in words). `pointer`: `fragment_root`, `generation`, `level`, and the fragment's `leaves` and `gaps`; every block was reached through its parent's key pointer (address, generation, level, first key, owner), as in an anchored walk. `sibling`: `objectid` and the `leaves` (bytenr, generation), head first; the file's items continue in the next leaf of the same tree, the extents of all of them cover the file exactly, none is newer than the INODE_ITEM, no joined leaf was written before the INODE_ITEM's last change, and no other scanned leaf continues the file differently. `log_root`: `subvolume` and `log_root_leaf`; the ROOT_ITEM of a log root tree names this log tree, and its key offset is the subvolume. `log_replay`: `base` (the state, as the superblock names it or `state:ID`), `base_state`, `base_root`; the same inode number and creation generation in that state's tree of the subvolume, over which the logged extents were laid as the kernel's replay would (a logged extent replaces its range, the logged INODE_ITEM gives the size). Without `log_replay` the content comes from the log alone. `parent_path`: the directory `objectid`, its `name_hex` and `parent`, the `leaves` holding that name, and, for the file's own directory, `dir_index_names_this_file` (whether a DIR_INDEX of that directory names this file under this name); the number has exactly one name and one creation generation in the scanned leaves of the tree. A join that would be ambiguous is not made: the artifact stays as `--orphans` gives it, and `problems` says why (`not joined with another leaf: …`, `no path: …`) |
...
- **7** (2026-09-21): `artifacts.in_current` is gone, as M4b announced. "Deleted since" is a
  ...
  pure function of the scan's tables and is not stored. A version-6 database is refused; rebuild
  it from the image.
  Added without a change of the DDL (2026-09-22, `recover --logs`): the `source_kind` value
  `log_tree`, the `source` form `log:BYTENR@GEN`, the joins `log_root` and `log_replay`, the
  `missing` reason `not_logged` (which lone log leaves of `--orphans` now get too), and `logs` and
  `log_trees` in `recovery_runs`.
